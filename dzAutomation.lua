...
return {
	...
	--domoticz.log('Change'..device.name..' level:'..device.level, domoticz.LOG_DEBUG)
		...
		elseif (device.name == 'Automation' and device.level == 40 and OperationSecondMode ~= 'Aanwezigheid Override') then
		-- Systeem status Weg
			domoticz.log('Iedereen is zojuist vertrokken.', domoticz.LOG_DEBUG)
			
			--Verwarming handmatig op 16 graden zetten
			domoticz.log('Changing thermostat program to temporary and temp to 16 degrees.', domoticz.LOG_INFO)
