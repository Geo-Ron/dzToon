...
local ModeSelectorSecond		       			= 'Automation Secundary'
	...
	local ModeSelectorSecondLevelKidVacPresOver	= 20	--Children school holiday ànd Presence detection Override
...
local DayTimeDevicesTimeRange                   = 'at 7:00-18:00'
...
return {
	logging = {
		...
		marker = 'dzAutomation '..scriptVersion
	},
	on = {
        timer = {
            --'Every minute',
	...
	execute = function(domoticz,device)
        ...
        if (device.name ~= ModeSelector)
		and (domoticz.devices(ModeSelector).level == ModeSelectorLevelManual) then
			...
			if (device.isTimer) then	--Start Timer Actions
				...
				if (domoticz.devices(HeatingScenesSelector).level == HeatingScenesLevelVacation)
				and (domoticz.devices(ModeSelector).level ~= ModeSelectorLevelVacation) then
				...
				elseif (domoticz.devices(HeatingScenesSelector).level ~= HeatingScenesLevelVacation)
				and	(domoticz.devices(ModeSelector).level == ModeSelectorLevelVacation) then
			...
			if (device.name == ModeSelector) then
				...
				elseif (device.level == ModeSelectorLevelVacation)
				and (domoticz.devices(HeatingScenesSelector).level ~= HeatingScenesLevelVacation) then
				...
				elseif (device.level == ModeSelectorLevelAway) then
					...
					-- Systeem status Weg
					if (OperationSecondMode ~= ModeSelectorSecondLevelPresOverr)
					and (OperationSecondMode ~= ModeSelectorSecondLevelKidVacPresOver) then 
						...
						domoticz.log('Fetched outside Temp from '..WU_Device..' with value: '..tostring(OutsideTemp)..' degrees.', domoticz.LOG_DEBUG)
						if (OutsideTemp < CurrentSetpoint) then
    		                local HeatingCorrection = domoticz.utils.round(30 / (CurrentSetpoint - OutsideTemp),1)
    		                domoticz.log('Heating correction when away '..tostring(HeatingCorrection)..' degrees. Temperature difference is '..tostring(CurrentSetpoint - OutsideTemp)..'degrees.', domoticz.LOG_DEBUG)
						    ...
						    domoticz.log('Outside is warmer than inside. No setpoint corrections performed.', domoticz.LOG_DEBUG) 
						end
    		            ...
    		            if domoticz.time.matchesRule(DayTimeDevicesTimeRange) then
							...
							local DayDevices = domoticz.devices().filter(DayTimeDevices)
							DayDevices.forEach(function(DayDevice)
                    								domoticz.log('Switching Daytime device '..DayDevice.name..' off.', domoticz.LOG_DEBUG)
			...
			elseif (device.name == HeatingScenesSelector) then
				...
				if (device.level == HeatingScenesLevelVacation)
				and (domoticz.devices(ModeSelector).level ~= ModeSelectorLevelVacation) then
				...
				-- Systeem status Vakantie. Wordt opgelegd door Toon
					...
					domoticz.devices(ModeSelector).switchSelector(ModeSelectorLevelVacation).silent()
					domoticz.devices(ModeSelectorSecond).switchSelector(ModeSelectorSecondLevelNormal)
				elseif (device.level ~= HeatingScenesLevelVacation)
				and (domoticz.devices(ModeSelector).level == ModeSelectorLevelVacation) then
