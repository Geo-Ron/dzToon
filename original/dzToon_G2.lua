--[[ 
    ...
    Author: waaren (https://www.domoticz.com/forum/memberlist.php?mode=viewprofile&u=13034)
    Scriptsource: https://www.domoticz.com/forum/viewtopic.php?t=26662

    Info by author:
        Created for one of the Dutch forum members who experienced a problem with the two Lua scripts controlling the communication between Toon and domoticz after updating to a recent domoticz version. This script uses the same uservariables as the original Lua script. many thanks to @johan1974 for testing, patience and constructive feedback. 
    ...
    execute = function(dz, item)
        ...
        elseif item.isHTTPResponse then -- Triggered by Toon response
            if item.trigger == toonResponse then -- return from getInfo()
                ...
                elseif isFree() then
                    logWrite(
                        "Toon did not return valid data. Please check what went wrong. Error code: " .. item.statusCode or
                            999,
                        dz.LOG_ERROR
                    )
